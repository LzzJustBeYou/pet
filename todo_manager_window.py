...
from PyQt5.QtCore import QDate, QSignalBlocker, Qt, QTime, QUrl, pyqtSignal
...
from holiday_calendar import CalendarDataError, HolidayCalendar, save_user_calendar
...
class TodoManagerWindow(QWidget):
    ...
    def start_new(self) -> None:
        ...
        self.date_edit.setDate(to_qdate(now.date()))
        self.no_date_check.setChecked(False)
        self.has_time_check.setChecked(False)
        self.time_edit.setTime(to_qtime(None))
    ...
    def _load_occurrence(self, occurrence: TodoOccurrence) -> None:
        ...
        self.date_edit.setDate(to_qdate(occurrence.due_date or local_now().date()))
        self.has_time_check.setChecked(occurrence.due_time is not None)
        self.time_edit.setTime(to_qtime(occurrence.due_time))
    ...
    def _sync_calendar_limits(self) -> None:
        if self.skip_holidays_check.isChecked() and self.work_calendar.covered_end:
            self.date_edit.setMaximumDate(to_qdate(self.work_calendar.covered_end))
            if self.work_calendar.covered_start:
                self.date_edit.setMinimumDate(to_qdate(self.work_calendar.covered_start))
        else:
            self.date_edit.setMinimumDate(QDate(2000, 1, 1))
            self.date_edit.setMaximumDate(QDate(9999, 12, 31))
    ...
    def _form_values(self):
        ...
        undated = self.no_date_check.isChecked()
        due_date = None if undated else from_qdate(self.date_edit.date())
        due_time = None
        if not undated and self.has_time_check.isChecked():
            due_time = snap_time_to_step(from_qtime(self.time_edit.time()))
